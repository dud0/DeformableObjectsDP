...
int numBodies = 27;               // default # of bodies in sim (can be overridden by command line switch --n=<N>)
...
float* hVel = 0;
float* hF = 0; // Fc
float* hForces = 0; //all Forces
float* hColor = 0;
float* hEdge = 0; // array of all edges
int nEdges = 54;
...
double demoTime = 60.0;              // length of each demo phase in sec
...
// utility
int computeNumEdges(int numBodies);
...
bool CompareResults(int numBodies)
{
    ...
    BodySystemCPU* nbodyCPU = new BodySystemCPU(numBodies, nEdges);
    nbodyCPU->setArray(BodySystem::BODYSYSTEM_POSITION, hPos);
    nbodyCPU->setArray(BodySystem::BODYSYSTEM_VELOCITY, hVel);
    nbodyCPU->setArray(BodySystem::BODYSYSTEM_F, hF);
    nbodyCPU->setArray(BodySystem::BODYSYSTEM_FORCES, hForces);
    nbodyCPU->setArray(BodySystem::BODYSYSTEM_EDGE, hEdge);
...
int computeNumEdges(int numBodies) {
	...
	for (i = 0; i < SCALE; i += tmpINC) {
		for (j = 0; j < SCALE; j += tmpINC) {
			for (k = 0; k < SCALE; k += tmpINC) {
				n += 3;
			}
			n -= 1;
		}
